alter table public.product_sales
  add column if not exists purchase_code text null,
...
create index if not exists idx_product_sales_purchase_code on public.product_sales (purchase_code);
create index if not exists idx_product_sales_order_public_code on public.product_sales (order_public_code);
create index if not exists idx_stock_movements_purchase_code on public.stock_movements (purchase_code);
create index if not exists idx_stock_movements_order_public_code on public.stock_movements (order_public_code);
...
create or replace function public.register_product_sale_tx(
  ...
  p_order_public_code text default null,
...
begin
  ...
  values (
    ...
    nullif(trim(p_purchase_code), ''),
    nullif(trim(p_order_public_code), ''),
    v_created_at
  )
